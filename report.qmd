...
To enhance our model’s ability to detect patterns in flight delays, we engineered several domain-informed features using only data available prior to departure. Specifically, we extracted the departure hour from the scheduled departure time (DEP_HOUR), created a weekend indicator (IS_WEEKEND), and combined origin and destination airports into a ROUTE variable to capture route-specific delay trends. To simplify modeling and reduce overfitting, we grouped infrequent categories for OP_CARRIER, ORIGIN, DEST, and ROUTE under the label “OTHER,” retaining only the top 10 (or 50 for routes) most frequent levels.

Additionally, because our dataset is imbalanced—with delays comprising only about 17% of all flights—we applied a threshold adjustment when making predictions with the Random Forest model. Rather than using the default 0.5 cutoff, we lowered it to 0.4 to improve the model’s ability to identify delayed flights. This approach is common in imbalanced classification settings and helped improve the model’s sensitivity to the minority class (delays), resulting in a better balance between specificity and recall.
